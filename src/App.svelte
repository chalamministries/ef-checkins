	  ...
	  {#each cards as card}
		<div class="card {card.color || 'green'}">
		  <div class="card-header">
			<img 
			  src={card.imageURL || 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"%3E%3Crect width="40" height="40" fill="%23E0E0E0"/%3E%3Ccircle cx="20" cy="16" r="7" fill="%239E9E9E"/%3E%3Cpath d="M8 36c0-8 6-12 12-12s12 4 12 12" fill="%239E9E9E"/%3E%3C/svg%3E'} 
			  class="card-avatar" 
			  alt=""
			...
			<div class="card-title">
			  ...
			  <p class="card-membership {card.membershipValid ? '' : 'card-alert'}">{card.membershipValid ? card.membership : card.message}</p>
			</div>
			<span class="card-time">
			  {new Date(card.timestamp).toLocaleTimeString()}
			</span>
		  </div>
		  {#if card.redAlert && card.redAlertTxt}
			<p class="card-alert"><span style="color: red; font-weight: 700">ALERT:</span> {card.redAlertTxt}</p>
		  {/if}
		  {#if card.yellowAlert && card.yellowAlertTxt}
			<p class="card-alert"><span style="color: #bf9500; font-weight: 700">WARNING:</span> {card.yellowAlertTxt}</p>
		  {/if}
		  {#if card.balanceDue && card.balance > 0}
